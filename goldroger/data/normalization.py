...
def _normalize_currency_code(raw: str) -> tuple[str, str]:
    """
    Normalize Yahoo currency codes to ISO-like majors.
    Returns (normalized_code, note).
    """
    raw_str = str(raw or "").strip()
    code = raw_str.upper()
    if raw_str in {"GBp", "GBX"}:
        return "GBP", f"{raw_str} quote normalized to GBP code"
    if code in {"GBX", "GBP"}:
        return "GBP", ""
    return code, ""
...
def build_data_normalization_audit(market_data: MarketData | None) -> dict[str, Any]:
    ...
    quote_ccy, q_note = _normalize_currency_code(q_ccy_raw)
    fin_ccy, f_note = _normalize_currency_code(f_ccy_raw)
    mcap_ccy, m_note = _normalize_currency_code(mcap_ccy_raw)
    ...
    try:
        if (
            ...
            implied_mcap = float(market_data.current_price) * float(market_data.shares_outstanding)
