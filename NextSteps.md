...
| 45 | Peer-breadth + valuation-consistency pass — added `consumer_hardware_ecosystem` archetype, controlled peer relaxation to prevent over-filtered Apple-like sets, strict exclusion of missing-EV/EBITDA peers from valuation counts/weights, peer-role labeling (`core/adjacent/qualitative`), peer-count-based comps weighting guardrails, live-vs-applied multiple diagnostics, expanded DCF diagnostics (including projected FCF CAGR), canonical thesis valuation reference, and `market_analysis_unavailable` source tags for missing TAM/growth | ✅ |
| 46 | Canonical-range + confidence guardrail pass — fixed stale peer-bucket reference (`semiconductors` + `semiconductor_equipment`), enforced thesis fair-value/point-estimate canonicalization against valuation sources, capped mega-cap comps weight to 35% when peer confidence is low or pre-blend DCF/comps dispersion >2.0x, enriched source confidence taxonomy (`unavailable` / `skipped`), and added regressions for Cisco bucket classification + high-dispersion comps-weight capping | ✅ |
| 47 | Apple valuation-calibration pass — introduced `premium_device_platform` target archetype, expanded deterministic hardware/ecosystem peer discovery, lowered Apple-like semiconductor cap to 20%, strengthened core-vs-adjacent peer weighting (platform/hardware above networking), switched trading-comps provenance to applied weighted peer source, surfaced DCF status and live-vs-applied EV/EBITDA in runtime diagnostics, upgraded model-signal logic for >30% downside cases, and generalized stale product-cycle phrasing in thesis/catalysts | ✅ |
| 48 | Scale-aware mega-cap peer integrity pass — enforced hard valuation-peer floor for mega-caps (max($100B, 5% target MCap)), split business-model vs scale similarity, forced tiny peers into qualitative-only mode, reweighted peers via business×scale×data×bucket, added effective peer count diagnostics/guardrails (with comps weight caps at weak diversification), surfaced valuation reliability metadata in CLI, and added regressions for mega-cap floor and low-effective-peer comps caps | ✅ |
